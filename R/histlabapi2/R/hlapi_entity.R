...
hlapi_entity<-function(entity.type, value, limit = 25,fields=NULL,coll.name=NULL,date=NULL,start.date=NULL,end.date=NULL,summary= FALSE, run=NULL,...){
...
for(i in 1:length(entity.type)){
  ...
  if(summary==TRUE){
    if(entity.type[i]=="countries"&is.null(search)) search<-paste0("entities?entity_type=eq.country&entity_name=ilike.*",v,"*")
    if(entity.type[i]=="countries"&is.null(search)) search<-paste0("entities?entity_type=eq.country&entity_name=ilike.*",v,"*")
    if(entity.type[i]=="countries"&is.null(search)) search<-paste0("entities?entity_type=eq.country&entity_name=ilike.*",v,"*")
    #"http://api.foiarchive.org/entities?entity_type=eq.person&corpus=eq.frus&entity_name=ilike.*nixon*"
  }
...
hlapi_entity('countries', value='Belize', summary = TRUE)
